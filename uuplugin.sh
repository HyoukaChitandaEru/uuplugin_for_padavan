...
download_url_init() {
   info=$(ip addr show br0)
   sn=$(echo "${info}" | grep "link/ether" | awk '{print $2}')
  UNINSTALL_DOWNLOAD_URL="http://router.uu.163.com/api/script/uninstall?type=openwrt"
  DOWNLOAD_URL="http://router.uu.163.com/api/plugin?type=openwrt-mipsel"
  return 0
}
...
start_acc() {
    ...
    if [ -e "${exefile}" ];then
        chmod u+x "${exefile}"
	uuver=$($exefile -v | awk -F 'version:' {'print $2'} | tr -d " ")
	logger -t "【uu加速器】" "运行${exefile} ${uuver}"
        ${exefile} "${confile}" >/dev/null 2>&1 &
        return
    fi

    if [ -f "${runtar}" ];then
    logger -t "【uu加速器】" "开始解压${runtar} - ${RUNNING_DIR}"
        tar zxvf "${runtar}" -C "${RUNNING_DIR}" 
        if [ "$?" = "0" ];then
            chmod u+x "${exefile}"
	    uuver=$($exefile -v | awk -F 'version:' {'print $2'} | tr -d " ")
    ...
    if [ -f "${backtar}" ];then
        ...
        if [ "$?" != "0" ];then
          ...
          logger -t "【uu加速器】" "开始解压${backtar} - ${RUNNING_DIR}"
	...
	logger -t "【uu加速器】" "运行${exefile} ${uuver}"
        ...
        # Download a new one. Next time ${runtar} will be used.
     logger -t "【uu加速器】" "找不到${runtar} 开始下载"
        download_acc "${runtar}"
        return
    fi
}
...
do
    ...
    if [ "$?" = "0" ];then
        ...
        # Pluin is not running now, so check it more frequently.
        sleep 5
    fi
if [ -f "/tmp/uu/.uuplugin_uuid" ] || [ -s "/home/root/.uuplugin_uuid" ] ;then
  if [ -f "/etc/storage/uu/.uuplugin_uuid" ] ; then
    tmp="$(md5sum /tmp/uu/.uuplugin_uuid | awk '{print $1;}')"
    root="$(md5sum /home/root/.uuplugin_uuid | awk '{print $1;}')"
    etc="$(md5sum /etc/storage/uu/.uuplugin_uuid | awk '{print $1;}')"
    echo $etc $tmp $root
     if [ "$tmp"x != "$etc"x ] || [ "$root"x != "$etc"x ]  ; then
       logger -t "【uu加速器】" "未上线？重新启动"
       killall uuplugin
       rm -rf /tmp/uu/.uuplugin_uuid /home/root/.uuplugin_uuid
       cp -rf "/etc/storage/uu/.uuplugin_uuid" "/tmp/uu/.uuplugin_uuid"
       cp -rf "/etc/storage/uu/.uuplugin_uuid" "/home/root/.uuplugin_uuid"
     fi
  fi
fi
[ ! -f /etc/storage/uu/.uuplugin_uuid ] && [ -s /tmp/uu/.uuplugin_uuid ] && cp -f /tmp/uu/.uuplugin_uuid /etc/storage/uu/.uuplugin_uuid
[ ! -f /etc/storage/uu/.uuplugin_uuid ] && [ -s /home/root/.uuplugin_uuid ] && cp -f /home/root/.uuplugin_uuid /etc/storage/uu/.uuplugin_uuid
done
}
